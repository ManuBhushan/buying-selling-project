...
export const getSearchedAdsWithoutLogin = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    const category =
      typeof req.query.category === "string" ? req.query.category : "others";
...
export const getSearchedAdsWithLogin = async (
  req: CustomRequest,
  res: Response,
) => {
  try {
...
export const unlikeRouter = async (req: CustomRequest, res: Response) => {
  const userId = req.userId?.id;
  ...
  try {
    ...
    res.status(200).json({ message: "Ad unliked successfully", result });
  } catch (error: any) {
    console.error("Error while unliking ad:", error);
    res.status(500).json({ error: "Unable to unlike ad" });
  }
};
